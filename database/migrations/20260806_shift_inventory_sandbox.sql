IF DB_NAME() NOT LIKE '%[_]Test'
    THROW 51000, 'Shift inventory migration is Sandbox-only.', 1;
GO
...
BEGIN
 CREATE TABLE dbo.shift_inventory_sessions(
  ...
  is_test BIT NOT NULL CONSTRAINT DF_sis_test DEFAULT 1,
  ...
  CONSTRAINT CK_sis_test CHECK(is_test=1)
 );
...
BEGIN
 CREATE TABLE dbo.inventory_discrepancies(
  ...
  resolved_at DATETIME2 NULL, adjustment_transaction_id BIGINT NULL, is_test BIT NOT NULL CONSTRAINT DF_id_test DEFAULT 1,
  ...
  CONSTRAINT CK_id_status CHECK(status IN('PENDING','RESOLVED')), CONSTRAINT CK_id_action CHECK(resolution_action IS NULL OR resolution_action IN('adjustment_in','adjustment_out','no_adjustment')), CONSTRAINT CK_id_test CHECK(is_test=1)
 );
...
BEGIN
 CREATE TABLE dbo.shift_inventory_audit_logs(
  ...
  created_at DATETIME2 NOT NULL CONSTRAINT DF_sial_created DEFAULT SYSDATETIME(), is_test BIT NOT NULL CONSTRAINT DF_sial_test DEFAULT 1,
  ...
  CONSTRAINT FK_sial_branch FOREIGN KEY(branch_id) REFERENCES dbo.branches(id), CONSTRAINT FK_sial_user FOREIGN KEY(actor_user_id) REFERENCES dbo.users(id), CONSTRAINT CK_sial_test CHECK(is_test=1)
 );
END
GO
