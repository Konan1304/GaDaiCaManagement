...
async function eligibility(executor,args){
  ...
  const r=await request.query(`
    ...
    WHERE es.employee_id=@employeeId AND es.branch_id=@branchId AND es.work_date=@date AND es.status<>'cancelled' AND es.is_test=1
    ...
    WHERE al.employee_id=@employeeId AND al.work_date=@date AND al.check_in_time IS NOT NULL AND al.status<>'absent' AND al.is_test=1
