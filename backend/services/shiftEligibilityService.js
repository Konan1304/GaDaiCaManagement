...
async function eligibility(executor,args){
  ...
  const selectedScheduleId=Number.isInteger(Number(args.scheduleId))&&Number(args.scheduleId)>0?Number(args.scheduleId):null;
  const request=executor.request().input("employeeId",sql.Int,employee.employeeId).input("branchId",sql.Int,employee.branchId).input("date",sql.Date,args.businessDate).input("operationShift",sql.VarChar(10),args.operationShift).input("scheduleId",sql.Int,selectedScheduleId).input("isTest",sql.Bit,isTest);
  ...
  const r=await request.query(`
    ...
    WHERE al.employee_id=@employeeId AND al.work_date=@date AND al.check_in_time IS NOT NULL AND al.status<>'absent' AND (al.is_test=@isTest OR (al.is_test=0 AND @isTest=1))
      AND (@scheduleId IS NULL OR al.schedule_id=@scheduleId)
