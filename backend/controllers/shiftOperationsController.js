...
async function current(req, res, next) { try {
  ...
  for (const operationShift of ['morning', 'evening']) {
    ...
    const reasons = [];
    if (state.employee?.userStatus !== 'active' || state.employee?.employeeStatus !== 'working') reasons.push('Tài khoản nhân viên không hoạt động');
    if (session) reasons.push('Ca này đã được mở');
    if (operationShift === 'evening') {
      const morning = sessions.recordsets[0].find(x => x.operationShift === 'morning');
      if (morning && morning.status !== 'LOCKED') reasons.push('Phải kết ca sáng trước khi mở ca tối');
    }
...
async function open(req,res,next){
  ...
  if(!employee?.employeeId||employee.userStatus!=='active'||employee.employeeStatus!=='working'){await tx.rollback();started=false;return fail(res,403,'Tài khoản nhân viên không hoạt động')}
  const existing=(await db.request().input('branchId',sql.Int,employee.branchId).input('date',sql.Date,businessDate).input('operation',sql.VarChar(10),operationShift).input('isTest',sql.Bit,isTest).query("SELECT TOP 1 id FROM dbo.shift_sessions WITH(UPDLOCK,HOLDLOCK) WHERE branch_id=@branchId AND business_date=@date AND operation_shift_code=@operation AND is_test=@isTest AND status NOT IN('cancelled','CANCELLED')")).recordset[0];
  if(existing){await tx.rollback();started=false;return fail(res,409,'Ca này đã được mở')}
  if(operationShift==='evening'){
    const morning=(await db.request().input('branchId',sql.Int,employee.branchId).input('date',sql.Date,businessDate).input('isTest',sql.Bit,isTest).query("SELECT TOP 1 status FROM dbo.shift_sessions WITH(UPDLOCK,HOLDLOCK) WHERE branch_id=@branchId AND business_date=@date AND operation_shift_code='morning' AND is_test=@isTest AND status NOT IN('cancelled','CANCELLED')")).recordset[0];
    if(morning&&morning.status!=='LOCKED'){await tx.rollback();started=false;return fail(res,409,'Phải kết ca sáng trước khi mở ca tối')}
  }
  const selectedScheduleId=Number(req.body.scheduleId||0)||null;
  const context=(await db.request().input('branchId',sql.Int,employee.branchId).input('employeeId',sql.Int,employee.employeeId).input('date',sql.Date,businessDate).input('operation',sql.VarChar(10),operationShift).input('scheduleId',sql.Int,selectedScheduleId).query(`SELECT TOP 1 es.id scheduleId,es.shift_id shiftId FROM dbo.employee_schedules es WHERE es.employee_id=@employeeId AND es.branch_id=@branchId AND es.work_date=@date AND (@scheduleId IS NULL OR es.id=@scheduleId) ORDER BY CASE WHEN es.id=@scheduleId THEN 0 ELSE 1 END;
    SELECT TOP 1 s.id shiftId FROM dbo.shifts s LEFT JOIN dbo.operation_shift_mappings m ON m.shift_id=s.id AND m.branch_id=@branchId AND m.is_active=1 WHERE s.status='active' AND (m.operation_shift_code=@operation OR (m.id IS NULL AND ((@operation='morning' AND s.shift_code IN('A','P1')) OR (@operation='evening' AND s.shift_code IN('B','P3'))))) ORDER BY CASE WHEN m.operation_shift_code=@operation THEN 0 ELSE 1 END,s.id`));
  const schedule=context.recordsets[0][0]||null,shiftId=schedule?.shiftId||context.recordsets[1][0]?.shiftId;
  if(!shiftId){await tx.rollback();started=false;return fail(res,409,'Chưa cấu hình ca làm việc cho chi nhánh')}
  const configured=await db.request().query("SELECT TRY_CONVERT(decimal(18,2),setting_value) AS amount FROM dbo.system_settings WHERE setting_key='DEFAULT_SHIFT_OPENING_CASH'");const requestedCash=req.body.openingCash==null?Number(configured.recordset[0]?.amount):Number(req.body.openingCash),openingCash=Math.round(requestedCash*100)/100;if(!Number.isFinite(openingCash)||openingCash<0){await tx.rollback();started=false;return fail(res,400,'Tiền đầu ca không hợp lệ')}
  const result=await db.request().input('branchId',sql.Int,employee.branchId).input('employeeId',sql.Int,employee.employeeId).input('scheduleId',sql.Int,schedule?.scheduleId||null).input('shiftId',sql.Int,shiftId).input('date',sql.Date,businessDate).input('operation',sql.VarChar(10),operationShift).input('userId',sql.Int,req.user.userId).input('cash',sql.Decimal(18,2),openingCash).input('note',sql.NVarChar(500),String(req.body.note||'').trim()||null).input('isTest',sql.Bit,isTest).query("INSERT dbo.shift_sessions(branch_id,employee_id,schedule_id,shift_id,business_date,opening_cash,status,note,operation_shift_code,leader_employee_id,opened_by,is_test,updated_at) OUTPUT INSERTED.id AS shiftSessionId VALUES(@branchId,@employeeId,@scheduleId,@shiftId,@date,@cash,'OPEN',@note,@operation,@employeeId,@userId,@isTest,SYSDATETIME())");
  const id=result.recordset[0].shiftSessionId;
  await record(db,{sessionId:id,branchId:employee.branchId,userId:req.user.userId,employeeId:employee.employeeId,action:'SHIFT_OPENED',newStatus:'OPEN',payload:{businessDate,operationShift,openingCash}});await tx.commit();started=false;res.status(201).json({success:true,message:'Mở ca thành công',data:{shiftSessionId:id,openingCash,businessDate,operationShift}});
