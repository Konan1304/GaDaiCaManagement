...
const parseShiftDisplayCode=value=>{let normalized=String(value||"").trim().toUpperCase().replace(/,/g,".");if(normalized==="AB")normalized="A+B";const explicit=/^(\d{1,2})(?:H(?:(\d{1,2}))?|:(\d{2}))-(\d{1,2})(?:H(?:(\d{1,2}))?|:(\d{2}))$/.exec(normalized);if(explicit){const start=Number(explicit[1])*60+Number(explicit[2]??explicit[3]??0),end=Number(explicit[4])*60+Number(explicit[5]??explicit[6]??0);return start>=0&&end<=1439&&start<end?{baseShiftCode:start<720?"A":start<1020?"P2":"P3",customRange:true}:null}const composite=/^(A\+B|P1\+P2\+P3|P1\+P2|P2\+P3)(?:([+-])((?:[1-4](?:\.5)?|0\.5)))?$/.exec(normalized);if(composite)return{baseShiftCode:composite[1].split("+")[0],components:composite[1].split("+"),operator:composite[2]||null,adjustmentHours:Number(composite[3]||0)};const match=/^(A|B|P1|P2|P3)(?:([+-])((?:[1-4](?:\.5)?|0\.5)))?$/.exec(normalized);return match?{baseShiftCode:match[1],operator:match[2]||null,adjustmentHours:Number(match[3]||0)}:null};
...
async function periods(req,res,next){try{const result=await (await getPool()).request().query(`${periodSelect} WHERE p.status<>'cancelled' ORDER BY p.week_start_date DESC,p.created_at DESC`);res.json({success:true,data:result.recordset})}catch(error){next(error)}}
async function periodsScoped(req,res,next){try{if(!req.managerBranchId)return periods(req,res,next);const result=await (await getPool()).request().input("branchId",sql.Int,req.managerBranchId).query(`${periodSelect} WHERE p.branch_id=@branchId AND p.status<>'cancelled' ORDER BY p.week_start_date DESC,p.created_at DESC`);res.json({success:true,data:result.recordset})}catch(error){next(error)}}
...
async function managerSchedules(req,res,next){try{
 ...
 const employeeWhere=["((e.status='working' AND u.status='active') OR EXISTS(SELECT 1 FROM employee_schedules esh WHERE esh.employee_id=e.id AND esh.work_date BETWEEN @from AND @to AND ISNULL(esh.is_test,0)=0))"],scheduleWhere=["es.work_date BETWEEN @from AND @to","ISNULL(es.is_test,0)=0"];
...
async function periodDetailWeekly(req,res,next){try{
 const result=await (await getPool()).request().input("id",sql.Int,req.params.id).query(`
  ...
  SELECT e.id AS employeeId,e.employee_code AS employeeCode,u.full_name AS fullName,pos.position_name AS positionName,e.position_id AS positionId,
   CONVERT(char(10),r.work_date,23) AS workDate,COALESCE(r.selection_code,s.shift_code) AS shiftCode,r.note
  FROM schedule_registration_periods rp JOIN employees e ON ((e.branch_id=rp.branch_id AND e.status='working')
    OR EXISTS(SELECT 1 FROM employee_shift_registrations rx WHERE rx.period_id=rp.id AND rx.employee_id=e.id)
    OR EXISTS(SELECT 1 FROM employee_schedules esx WHERE esx.employee_id=e.id AND esx.branch_id=rp.branch_id AND esx.work_date BETWEEN rp.week_start_date AND rp.week_end_date AND ISNULL(esx.is_test,0)=0))
  JOIN users u ON u.id=e.user_id JOIN positions pos ON pos.id=e.position_id
