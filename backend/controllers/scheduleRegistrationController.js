...
const periodSelect=`SELECT p.id AS periodId,p.branch_id AS branchId,b.branch_code AS branchCode,b.branch_name AS branchName,
 p.title,p.week_start_date AS weekStartDate,p.week_end_date AS weekEndDate,p.registration_open_at AS registrationOpenAt,
 p.registration_close_at AS registrationCloseAt,p.status,p.note,p.created_at AS createdAt,
...
function validatePeriodInput(body){
  if(!Number.isInteger(Number(body.branchId))||Number(body.branchId)<=0)return "Chi nhánh không hợp lệ";
  if(!String(body.title||"").trim())return "Tên đợt đăng ký là bắt buộc";
  if(!validDate(body.weekStartDate))return "Ngày bắt đầu không hợp lệ";
  if(!validDate(body.weekEndDate))return "Ngày kết thúc không hợp lệ";
  const start=new Date(`${body.weekStartDate}T00:00:00Z`),end=new Date(`${body.weekEndDate}T00:00:00Z`);
  if(end<start)return "Ngày kết thúc phải bằng hoặc sau ngày bắt đầu";
  if((end-start)/864e5>31)return "Một đợt đăng ký không được dài quá 32 ngày";
  const open=new Date(body.registrationOpenAt),close=new Date(body.registrationCloseAt);
  if(Number.isNaN(open.getTime())||Number.isNaN(close.getTime()))return "Thời gian đăng ký không hợp lệ";
  if(close<=open)return "Thời gian đóng đăng ký phải sau thời gian mở";
  if(close>new Date(`${body.weekStartDate}T23:59:59`))return "Thời gian đóng đăng ký phải trước hoặc bằng ngày bắt đầu tuần";
  return null;
}

async function current(req,res,next){try{const pool=await getPool(),employee=await employeeFor(pool,req.user.userId,false);if(!employee)return res.json({success:true,data:null});const result=await pool.request().input("branchId",sql.Int,employee.branchId).input("employeeId",sql.Int,employee.employeeId).query(`
 ${periodSelect} WHERE p.branch_id=@branchId AND p.status IN ('open','locked','published') ORDER BY CASE p.status WHEN 'open' THEN 0 WHEN 'locked' THEN 1 ELSE 2 END,p.week_start_date DESC;
 ${shiftSelect};
...
async function createPeriod(req,res,next){try{const b=req.body;if(!checkWeek(b.weekStartDate,b.weekEndDate)||!b.title||!b.branchId||!b.registrationOpenAt||!b.registrationCloseAt)return fail(res,400,"Thông tin đợt đăng ký không hợp lệ");if(new Date(b.registrationCloseAt)<=new Date(b.registrationOpenAt))return fail(res,400,"Hạn đóng phải sau thời gian mở");const result=await (await getPool()).request().input("branchId",sql.Int,b.branchId).input("title",sql.NVarChar(200),b.title.trim()).input("start",sql.Date,b.weekStartDate).input("end",sql.Date,b.weekEndDate).input("openAt",sql.DateTime2,b.registrationOpenAt).input("closeAt",sql.DateTime2,b.registrationCloseAt).input("createdBy",sql.Int,req.user.userId).input("note",sql.NVarChar(1000),b.note||null).query(`DECLARE @status varchar(20)=CASE WHEN SYSDATETIME() BETWEEN @openAt AND @closeAt THEN 'open' ELSE 'draft' END;
INSERT INTO schedule_registration_periods(branch_id,title,week_start_date,week_end_date,registration_open_at,registration_close_at,status,created_by,note)
OUTPUT INSERTED.id AS periodId,INSERTED.status
VALUES(@branchId,@title,@start,@end,@openAt,@closeAt,@status,@createdBy,@note)`);const created=result.recordset[0];res.status(201).json({success:true,message:created.status==="open"?"Tạo và mở đợt đăng ký thành công":"Tạo đợt nháp thành công. Đợt chưa hiện cho nhân viên cho đến khi được mở.",data:created})}catch(error){next(error)}}
...
async function publishDraft(req,res,next){const pool=await getPool(),tx=new sql.Transaction(pool);let started=false;try{
 ...
 await new sql.Request(tx).input("branchId",sql.Int,p.branch_id).input("start",sql.Date,p.week_start_date).input("end",sql.Date,p.week_end_date).query("DELETE FROM employee_schedules WHERE branch_id=@branchId AND work_date BETWEEN @start AND @end");
 await new sql.Request(tx).input("periodId",sql.Int,p.id).input("branchId",sql.Int,p.branch_id).input("userId",sql.Int,req.user.userId).query(`INSERT INTO employee_schedules(employee_id,shift_id,branch_id,work_date,work_position,note,status,created_by)
   SELECT employee_id,shift_id,@branchId,work_date,work_position,note,'scheduled',@userId FROM schedule_draft_assignments WHERE period_id=@periodId`);
...
async function builderSave(req,res,next){const pool=await getPool(),tx=new sql.Transaction(pool);let started=false;try{const list=Array.isArray(req.body.schedules)?req.body.schedules:[];await tx.begin();started=true;const pr=await new sql.Request(tx).input("id",sql.Int,req.params.periodId).query("SELECT * FROM schedule_registration_periods WITH(UPDLOCK) WHERE id=@id");const p=pr.recordset[0];if(!p){await tx.rollback();started=false;return fail(res,404,"Không tìm thấy đợt đăng ký")}if(!["locked","published"].includes(p.status)){await tx.rollback();started=false;return fail(res,409,"Cần khóa đăng ký trước khi xếp lịch")}const shifts=await new sql.Request(tx).query(shiftSelect),byCode=new Map(shifts.recordset.map(x=>[x.shiftCode,x.shiftId]));const employees=await new sql.Request(tx).input("branchId",sql.Int,p.branch_id).query("SELECT id FROM employees WHERE branch_id=@branchId AND status='working'"),ids=new Set(employees.recordset.map(x=>Number(x.id)));for(const x of list)if(!ids.has(Number(x.employeeId))||!validDate(x.workDate)||x.workDate<new Date(p.week_start_date).toISOString().slice(0,10)||x.workDate>new Date(p.week_end_date).toISOString().slice(0,10)||!byCode.has(x.shiftCode)){await tx.rollback();started=false;return fail(res,400,"Dữ liệu xếp lịch không hợp lệ")}
 await new sql.Request(tx).input("branchId",sql.Int,p.branch_id).input("start",sql.Date,p.week_start_date).input("end",sql.Date,p.week_end_date).query("DELETE FROM employee_schedules WHERE branch_id=@branchId AND work_date BETWEEN @start AND @end");
 for(const x of list){await new sql.Request(tx).input("employeeId",sql.Int,x.employeeId).input("shiftId",sql.Int,byCode.get(x.shiftCode)).input("branchId",sql.Int,p.branch_id).input("date",sql.Date,x.workDate).input("position",sql.NVarChar(100),x.workPosition||null).input("note",sql.NVarChar(500),x.note||null).input("userId",sql.Int,req.user.userId).query("INSERT INTO employee_schedules(employee_id,shift_id,branch_id,work_date,work_position,note,status,created_by) VALUES(@employeeId,@shiftId,@branchId,@date,@position,@note,'scheduled',@userId)")}
