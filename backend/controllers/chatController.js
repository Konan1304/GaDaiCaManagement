...
async function channels(req,res,next){try{
  ...
  const r=await q.query(`SELECT c.id channelId,c.branch_id branchId,c.channel_type channelType,c.name,(SELECT COUNT(*) FROM chat_messages m WHERE m.channel_id=c.id AND m.is_deleted=0 AND m.is_test=@isTest AND m.id>COALESCE((SELECT last_read_message_id FROM chat_read_states rs WHERE rs.channel_id=c.id AND rs.user_id=${Number(req.user.userId)}),0)) unreadCount FROM chat_channels c WHERE c.is_active=1 AND c.is_test=@isTest${b?' AND c.branch_id=@b':''} ORDER BY c.channel_type`);
...
async function messages(req,res,next){try{
  ...
  const page=Math.max(1,+req.query.page||1),size=Math.min(50,Math.max(1,+req.query.pageSize||30)),r=await p.request().input("id",sql.Int,c.id).input("offset",sql.Int,(page-1)*size).input("size",sql.Int,size).input("isTest",sql.Bit,isTest).query("SELECT m.id messageId,m.message_type messageType,m.content,m.reply_to_id replyToId,m.action_url actionUrl,m.created_at createdAt,u.full_name senderName FROM chat_messages m LEFT JOIN users u ON u.id=m.sender_user_id WHERE m.channel_id=@id AND m.is_deleted=0 AND m.is_test=@isTest ORDER BY m.id DESC OFFSET @offset ROWS FETCH NEXT @size ROWS ONLY");
