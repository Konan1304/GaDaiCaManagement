...
async function seed(pool) {
  ...
  try {
    ...
    await q(`MERGE dbo.positions t USING (VALUES ('MANAGER',N'Quản lý'),('SUPERVISOR',N'Giám sát'),('WAREHOUSE',N'Nhân viên kho'),('ACCOUNTANT',N'Kế toán'),('CASHIER',N'Nhân viên thu ngân'),('KITCHEN',N'Nhân viên bếp'),('SERVICE',N'Nhân viên phục vụ')) s(code,name) ON t.position_code=s.code WHEN MATCHED THEN UPDATE SET position_name=s.name WHEN NOT MATCHED THEN INSERT(position_code,position_name) VALUES(s.code,s.name);`);
