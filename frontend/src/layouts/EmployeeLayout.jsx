...
export default function EmployeeLayout(){
  ...
  const [profile,setProfile]=useState(null),[loading,setLoading]=useState(true),[profileError,setProfileError]=useState("");
  const page=location.pathname.split("/").pop(),home=page==="home";
  useEffect(()=>{profileApi.get().then(r=>setProfile(r.data)).catch(e=>setProfileError(e.response?.data?.message||"Không kết nối được máy chủ")).finally(()=>setLoading(false))},[]);
  ...
  return <div className="emp-app">
    <header className={`emp-header ${home?"emp-header-home":""}`}>
      {home?<><div className="emp-avatar">{profile?.avatarUrl?<img src={profile.avatarUrl} alt=""/>:initials}</div><div className="emp-greeting"><small>Xin chào</small><strong>{profile?.fullName|| (loading?"Đang tải...":"Nhân viên")}</strong></div></>:<>
    ...
    <main className="emp-content"><Outlet context={{profile,setProfile,loading,profileError,logout}}/></main>
