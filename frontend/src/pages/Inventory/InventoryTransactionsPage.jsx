...
import {inventoryOverviewApi,productApi} from "../../api/services";
...
export default function InventoryTransactionsPage({branches=[],defaultBranchId="",period,onChanged}){
  const defaults=useMemo(()=>({branchId:String(defaultBranchId||branches[0]?.id||""),dateFrom:period?.start||"",dateTo:period?.end||"",search:"",page:1,limit:30}),[defaultBranchId,branches,period]);
  const [filters,setFilters]=useState(defaults),[rows,setRows]=useState([]),[page,setPage]=useState({page:1,total:0,totalPages:1});
  const [products,setProducts]=useState([]),[mode,setMode]=useState(""),[selected,setSelected]=useState(null),[error,setError]=useState(""),[message,setMessage]=useState(""),[busy,setBusy]=useState(false);
  ...
  useEffect(()=>{load(filters)},[filters.branchId,filters.dateFrom,filters.dateTo,filters.page]);
  ...
  return <div className="inventory-transactions-tab">
    ...
    <div className="card inventory-table-card"><div className="inventory-table-head"><div><h3>Lịch sử nhập - xuất</h3><p>{page.total||0} giao dịch trong khoảng thời gian đã chọn</p></div></div>
    ...
    {mode==="import"&&<div className="manager-modal-backdrop inventory-import-overlay"><div className="inventory-import-shell"><button className="inventory-overlay-close" onClick={()=>{setMode("");load();onChanged?.()}}><FiX/></button><ImportInventoryPage/></div></div>}
    ...
    {mode==="export"&&<div className="manager-modal-backdrop"><div className="employee-modal category-modal"><header><div><h2>Xuất hàng</h2><p>Ghi nhận xuất kho và tự động trừ tồn tại chi nhánh.</p></div><button onClick={()=>setMode("")}><FiX/></button></header><form className="category-form" onSubmit={submitExport}>
      ...
      <label>Ghi chú<textarea value={form.reason} onChange={e=>setForm({...form,reason:e.target.value})}/></label>
