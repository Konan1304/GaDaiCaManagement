...
export default function InventoryPage(){
  ...
  useEffect(()=>{let active=true;setLoading(true);setError("");inventoryOverviewApi.list(filters).then(r=>{if(!active)return;setData(r.data);if(!filters.branchId&&r.data.selectedBranchId)setFilters(v=>({...v,branchId:String(r.data.selectedBranchId)}))}).catch(e=>{if(!active)return;setError(e.response?.data?.message||"Không tải được dữ liệu tổng kho");setData(v=>({...v,items:[],summary:{},pagination:{page:1,total:0,totalPages:1}}))}).finally(()=>active&&setLoading(false));return()=>{active=false}},[filters.branchId,filters.dateFrom,filters.dateTo,filters.search,filters.page]);
