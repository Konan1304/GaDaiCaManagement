...
export default function AttendancePage(){
 ...
 const loadToday=useCallback((date,scheduleId)=>employeeApi.attendanceToday(date,scheduleId).then(r=>{setData(r.data);setSelectedDate(r.data.businessDate||date||"");if(isSandbox&&(r.data.businessDate||date))localStorage.setItem("sandbox-operation-date",r.data.businessDate||date)}),[isSandbox]);
 useEffect(()=>{let active=true;setLoading(true);setError("");const request=isSandbox?Promise.all([employeeApi.attendanceTestSchedules(),employeeApi.attendanceToday()]).then(async([optionsResult,todayResult])=>{if(!active)return;const options=optionsResult.data||[];setScheduleOptions(options);const remembered=localStorage.getItem("sandbox-active-schedule-id")||localStorage.getItem("sandbox-attendance-schedule-id");const selected=options.find(item=>String(item.scheduleId)===remembered)||options.find(item=>Number(item.scheduleId)===Number(todayResult.data?.schedule?.scheduleId))||options[0];if(selected){setSelectedScheduleId(String(selected.scheduleId));localStorage.setItem("sandbox-active-schedule-id",String(selected.scheduleId));localStorage.setItem("sandbox-attendance-schedule-id",String(selected.scheduleId));localStorage.setItem("sandbox-operation-date",selected.workDate);await loadToday(selected.workDate,selected.scheduleId)}else{setData(todayResult.data);setSelectedDate(todayResult.data?.businessDate||"")}}):loadToday();request.catch(e=>active&&setError(e.response?.data?.message||"Không tải được dữ liệu chấm công")).finally(()=>active&&setLoading(false));return()=>{active=false}},[isSandbox,loadToday]);
 async function changeTestSchedule(value){const selected=scheduleOptions.find(item=>String(item.scheduleId)===value);if(!selected)return;setSelectedScheduleId(value);setSelectedDate(selected.workDate);localStorage.setItem("sandbox-active-schedule-id",value);localStorage.setItem("sandbox-attendance-schedule-id",value);localStorage.setItem("sandbox-operation-schedule-id",value);localStorage.setItem("sandbox-shift-inventory-schedule-id",value);localStorage.setItem("sandbox-operation-date",selected.workDate);localStorage.setItem("sandbox-shift-inventory-date",selected.workDate);setLoading(true);setError("");setMessage("");try{await loadToday(selected.workDate,selected.scheduleId)}catch(e){setError(e.response?.data?.message||"Không tải được lịch chấm công")}finally{setLoading(false)}}
