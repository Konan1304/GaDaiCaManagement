...
export default function OperationShiftPage(){
  ...
  async function open(operationShift){try{const result=await operationApi.open({operationShift,...(isSandbox&&sandboxDate?{businessDate:sandboxDate,scheduleId:Number(selectedScheduleId)}:{})});setMessage(result.message);await load()}catch(e){setError(apiError(e))}}
  ...
  return <div className="operation-employee">{message&&<div className="emp-toast">{message}</div>}{error&&<div className="emp-error">{error}</div>}<section className="emp-card operation-heading"><FiClock/><div><small>Ngày nghiệp vụ</small><h2>{data?.businessDate||"Chưa xác định"}</h2></div></section>
    {isSandbox&&<section className="emp-card operation-schedule-picker"><div><strong>Chọn ngày theo lịch làm</strong><small>Chỉ hiển thị lịch chính thức đã công bố của bạn.</small></div><select value={selectedScheduleId} onChange={event=>changeSchedule(event.target.value)} disabled={!scheduleOptions.length}><option value="">{scheduleOptions.length?"Chọn ngày và ca làm":"Không có lịch đã công bố"}</option>{scheduleOptions.map(item=><option key={item.scheduleId} value={item.scheduleId}>{scheduleLabel(item)}</option>)}</select></section>}
