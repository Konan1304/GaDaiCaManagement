...
export default function ShiftInventoryAdminPage({ embedded = false }) {
  ...
  const load = async () => {
    try {
      ...
      if (!branch) {
        const branchWithData = sessionResult.data.find((item) => item.operationShift !== 'baseline');
        const preferredBranchId = branchWithData?.branchId || optionResult.data.branches[0]?.branchId;
        if (preferredBranchId) setBranch(String(preferredBranchId));
      }
  ...
  return <div className={`si-admin ${embedded ? 'si-admin-embedded' : ''}`}>
    ...
    {selected && <section className="si-card si-detail"><div className="si-section-title"><div><h2>Phiên #{selected.id} · {selected.branchName}</h2><p>{String(selected.business_date).slice(0, 10)} · Ca {selected.operation_shift_code === 'morning' ? 'sáng' : 'chiều'}</p></div><button className="si-secondary" onClick={() => setSelected(null)}>Đóng</button></div>
      ...
      <div className="si-table"><div className="si-row si-stock-row si-head"><b>Sản phẩm</b><b>ĐVT</b><b>Tồn hệ thống</b><b>Tồn nhân viên đếm</b><b>Chênh lệch</b><b>Ghi chú</b><b>Trạng thái</b></div>{selected.items.map((item) => { const hasClosing=item.closing_actual_quantity!=null;const actual=hasClosing?Number(item.closing_actual_quantity):Number(item.opening_actual_quantity??item.actual_received_quantity??0);const expected=hasClosing?Number(item.opening_actual_quantity??item.declared_handover_quantity??0)+Number(item.imported_quantity_in_shift||0)+Number(item.adjustment_in_quantity||0)-Number(item.special_export_quantity_in_shift||0)-Number(item.adjustment_out_quantity||0):Number(item.declared_handover_quantity??0);const roundedExpected=Math.round(expected*1000)/1000,roundedActual=Math.round(actual*1000)/1000,diff=Math.round((roundedActual-roundedExpected)*1000)/1000; return <div className={`si-row si-stock-row ${diff ? 'si-row-danger' : ''}`} key={item.id}><strong>{item.productName}</strong><span>{item.unitName}</span><span>{roundedExpected}</span><span>{roundedActual}</span><b className={diff ? 'danger' : ''}>{diff}</b><span>{item.receiving_note || item.closing_note || '—'}</span><span className={`si-pill ${diff ? 'danger' : 'ok'}`}>{diff ? 'Lệch' : 'Khớp'}</span></div>; })}</div>
