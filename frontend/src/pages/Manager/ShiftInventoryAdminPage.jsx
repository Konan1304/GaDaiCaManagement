...
export default function ShiftInventoryAdminPage({ embedded = false }) {
  ...
  return <div className={`si-admin ${embedded ? 'si-admin-embedded' : ''}`}>
    ...
    {selected && <section className="si-card si-detail"><div className="si-section-title"><div><h2>Phiên #{selected.id} · {selected.branchName}</h2><p>{String(selected.business_date).slice(0, 10)} · Ca {selected.operation_shift_code === 'morning' ? 'sáng' : 'chiều'}</p></div><button className="si-secondary" onClick={() => setSelected(null)}>Đóng</button></div>
      ...
      <div className="si-table"><div className="si-row si-head"><b>Sản phẩm</b><b>ĐVT</b><b>Ca trước</b><b>Tồn đầu</b><b>Nhập</b><b>Xuất</b><b>Tồn cuối</b><b>Chênh lệch nhận</b><b>Ghi chú</b><b>Trạng thái</b></div>{selected.items.map((item) => { const diff = Number(item.receiving_difference_quantity || 0); const imported = item.liveImportedQuantity ?? item.imported_quantity_in_shift ?? 0; const exported = item.liveExportedQuantity ?? item.special_export_quantity_in_shift ?? 0; return <div className={`si-row ${diff ? 'si-row-danger' : ''}`} key={item.id}><strong>{item.productName}</strong><span>{item.unitName}</span><span>{item.declared_handover_quantity ?? '—'}</span><span>{item.opening_actual_quantity ?? '—'}</span><span>+{imported}</span><span>-{exported}</span><span>{item.closing_actual_quantity ?? '—'}</span><b className={diff ? 'danger' : ''}>{diff}</b><span>{item.receiving_note || item.closing_note || '—'}</span><span className={`si-pill ${diff ? 'danger' : 'ok'}`}>{diff ? 'Lệch' : 'Khớp'}</span></div>; })}</div>
