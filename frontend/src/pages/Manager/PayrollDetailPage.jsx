...
export default function PayrollDetailPage(){
 ...
 return <section className="payroll-detail"><button className="payroll-back" onClick={()=>navigate(`/manager/payrolls?month=${month}`)}><FiArrowLeft/> Quay lại bảng lương</button>
  ...
  <div className="payroll-detail-grid"><div className="card payroll-days"><h2>Chấm công thực tế</h2><table className="data-table"><thead><tr><th>Ngày</th><th>Giờ vào</th><th>Giờ ra</th><th>Tổng giờ</th></tr></thead><tbody>{data.days.length?data.days.map(day=><tr key={day.workDate}><td>{new Date(`${day.workDate}T00:00:00`).toLocaleDateString("vi-VN")}</td><td>{new Date(day.checkIn).toLocaleTimeString("vi-VN",{hour:"2-digit",minute:"2-digit",timeZone:"UTC"})}</td><td>{new Date(day.checkOut).toLocaleTimeString("vi-VN",{hour:"2-digit",minute:"2-digit",timeZone:"UTC"})}</td><td><b>{Number(day.totalHours).toFixed(2)} giờ</b></td></tr>):<tr><td colSpan="4" className="payroll-empty">Tháng này chưa có lượt chấm công vào/ra hợp lệ</td></tr>}</tbody></table></div>
