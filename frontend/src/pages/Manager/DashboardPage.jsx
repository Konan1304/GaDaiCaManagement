...
export default function DashboardPage(){
 const {user,role}=getSession(),[data,setData]=useState(null),[branchId,setBranchId]=useState(null),[loading,setLoading]=useState(true),[error,setError]=useState("");
 const load=async selected=>{setLoading(true);setError("");try{const response=await dashboardApi.get(selected?{branchId:selected}:undefined);setData(response.data)}catch(e){setError(e.response?.data?.message||"Không tải được dữ liệu tổng quan.")}finally{setLoading(false)}};
 ...
 return <div className="real-dashboard">
  <div className="welcome dashboard-welcome"><div><h2>Xin chào, {user?.name||"Quản lý"}! 👋</h2><p>Tình hình vận hành ngày {date(data?.businessDate)}.</p></div><div className="dashboard-actions">{role==="admin"&&<label className="dashboard-branch-filter"><small>Phạm vi số liệu</small><select value={branchId??""} onChange={e=>setBranchId(e.target.value)}><option value="">Toàn hệ thống (gộp các chi nhánh)</option>{data?.branches?.map(item=><option key={item.branchId} value={item.branchId}>{item.branchName}</option>)}</select></label>}<Link className="btn btn-primary" to="/manager/operations/dashboard">Xem báo cáo ca</Link></div></div>
  {error&&<div className="manager-form-error">{error}</div>}
  <section className="dashboard-live-stats">
   <Link className="dashboard-live-stat" to="/manager/operations/dashboard" aria-label="Xem doanh thu hôm nay"><i className="revenue"><FiDollarSign/></i><span><small>Doanh thu hôm nay</small><b>{money(summary.totalRevenue)}</b><em>{Number(summary.orderCount||0).toLocaleString("vi-VN")} đơn đã báo cáo</em></span></Link>
   ...
   <Link className="dashboard-live-stat" to="/manager/attendance" aria-label="Xem chấm công hôm nay"><i className="people"><FiUsers/></i><span><small>Chấm công hôm nay</small><b>{summary.checkedInEmployees}/{summary.scheduledEmployees}</b><em>nhân viên đã chấm công</em></span></Link>
  ...
  <section className="dashboard-lower-grid">
   ...
   <article className="card dashboard-shift-status"><div className="card-head"><div><h3>Ca hôm nay</h3><p>Trạng thái mở và kết ca</p></div></div>{shiftCards.map(item=><section key={item.code}><header><b>{shiftName(item.code)}</b><span className={item.open?"open":item.closed?"closed":"pending"}>{item.open?"Đang mở":item.closed?"Đã kết ca":"Chưa mở"}</span></header>{item.items.length?item.items.slice(0,3).map((shift,index)=><p key={`${shift.operationShift}-${index}`}><span>{shift.leaderName||"Chưa xác định người mở"}</span><small>{statusName(shift.status)}</small></p>):<p><span>Chưa có ca vận hành</span></p>}</section>)}</article>
  </section>
 </div>;
}
